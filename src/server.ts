/**
 * 《全巢之力》Web 服务：WebSocket（ws 库）+ Node http 静态托管 + REST API。
...
/** 战役包目录：WANGDACHUI_CAMPAIGN=ants → assets/campaigns/ants/（不存在则返回 null） */
...
function loadDefaultCard(stateDir?: string): CharacterCard | null {
	...
	const example = resolve(root, "assets/campaigns/ants/card-ants.json");
...
function buildExportWordHtml(md: string): string {
	...
	const body = md
		...
		.join("\n");
	return `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><title>全巢之力 对话记录</title></head><body style="font-family:'Microsoft YaHei',sans-serif;max-width:760px;margin:24px auto;color:#222;">${body}</body></html>`;
}
...
function buildExtraRules(st: SessionState): string {
	...
	if (/拒绝|不去|算了|不干|不要|走开|离开|不想/.test(last)) {
		rules.push(
			"【偏离处理】玩家可能拒绝了某个邀约/线索，或干脆走开。后果要真实（对方失望、线索关闭、处境更险），但关键剧情节点不能被跳过：用世界因果让关键事件以另一种方式再次找上玩家（如无视显灵→法阵越来越亮、越来越频繁；不理献祭→那一派改而自我绝食）。永远给玩家重新选择的机会。",
		);
	}
	return rules.join("\n");
}
...
server.listen(PORT, () => {
	console.log(`✦ 全巢之力 已启动：http://127.0.0.1:${PORT}`);
	console.log(
