...
/** 美术方向：与 assets/campaigns/ants/art/README.md 保持一致 */
const ART_DIRECTION = `实拍微距摄影，真实照片质感，浅景深，钠灯的暖色侧光，空气里的浮尘，被放大的细部，自然主义写实，电影感，宇宙恐怖氛围。
铁律：神（人类）永远不能出现在画面里——不许出现手指、手、人影、人体局部、家具或任何生物（除了蚂蚁与昆虫）；只允许用画框外压下来的巨大暗影、暖光或失焦的暗部来暗示其存在。
画面中绝对不能出现任何文字、字幕、水印、对话框、UI 元素。构图把蚂蚁放在画面中间，四周留出可裁切的余量。
必须是微观世界的写实照片质感，不要 3D 渲染感、不要插画感、不要概念设定图感。`;

const PROMPT_SYSTEM = `你是《全巢之力》的配图提示词作者。这部作品讲一个下班回家的普通人被一窝蚂蚁奉为神明，故事发生在城市楼道的台阶砖缝之间。

把你收到的剧情片段，改写成一句可以直接交给文生图模型的画面描述。写成一整句连贯的话（不要换行、不要引号、不要任何括号或字段标签、不要解释），依次交代三件事：这句话里最显眼的那些蚂蚁在做什么、在什么位置、什么姿态；周围的环境（台阶／砖缝／糖粒／水／蒸汽／香灰法阵）；暖色侧光从哪个方向照过来。最后缀上"浅景深，浮尘，写实电影感"。

硬性要求：
1. 必须出现"蚂蚁""工蚁""蚁群"这类词——这部作品的主角永远是蚂蚁，画面里必须看得到它们；
2. 全句 60-120 字，信息密度要高，不要只写环境不写主体；
3. 严格遵守美术方向：${ART_DIRECTION}
4. 不要写心理活动、对白、比喻；不要出现人、手、手指、人影或人体局部。

如果剧情里没有明显画面（纯对话、纯内心活动），就画"这段剧情正在发生的那间台阶／砖缝的空镜 + 至少一只蚂蚁"，不要省略主体。`;
  ...
  private async buildPrompt(narrative: string): Promise<string> {
    ...
    for (const maxTokens of [600, 1800]) {
      try {
        ...
        // 合格线：够长、且点到了蚂蚁——否则出的图会是一张没有主角的砖缝纹理
        if (line.length >= 25 && /蚁|ant/i.test(line)) return line.slice(0, 300);
      } catch {
        /* 换更大预算再试 */
      }
    }
    ...
    return fallback ? `微距昆虫摄影，画面里有一群工蚁。${fallback}` : "";
  }
