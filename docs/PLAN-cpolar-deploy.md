...
| 项目 | 完整可用 | `cd /path/to/quanchaozhili && ls src/server.ts` |
...
- [x] cpolar 穿透成功，拿到公网地址（cpolar v3.3.12 便携版）
...
免费版 cpolar 分配的是随机子域名（形如 `https://xxxxxxxx.cpolar.cn`），**每次重启穿透后地址都会变**，需要重新记录。重启命令：`cpolar http 7620`。

> 具体地址不入库——穿透地址等于从公网直通本机，写进仓库等于把家里的机器挂到网上。需要时在本机 `cpolar` 的状态页（默认 `http://127.0.0.1:4040`）查看当前地址。重启前先 `taskkill //F //IM cpolar.exe`，避免 4040 状态端口冲突。
